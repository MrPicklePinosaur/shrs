...
impl Plugin for AnalyticsPlugin {
    fn init(&self, shell: &mut ShellConfig) -> Result<()> {
        shell.builtins.insert("analytics", analytics_builtin);
...
fn record_dir_change(_sh: &Shell, _cd_ctx: &ChangeDirCtx) -> Result<()> {
    Ok(())
}

fn most_common_commands(_sh: &Shell, cmd_ctx: &BeforeCommandCtx) -> anyhow::Result<()> {
